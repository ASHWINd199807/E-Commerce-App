...
const isAuthenticated = async(req, res , next) => {
  ...
  if (token) {
    ...
    res.redirect("/login"); // see 2.41.32 , REDIRECT get method pe hoga
  }
};
...
app.post("/login", async(req,res)=>{
   ...
   if (!user) { return res.redirect("./register") } // if email not in database , IF WANT TO PAS TEXT USE res.render()
   ...
   res.redirect("/"); // can now redirect to any page   , as redirected to "/" and as isAuthenticated so, logout is rendered ! 
...
app.post("/register", async(req, res) => {
  ...
  if (user){
    ...
    return res.redirect("/login")  // can send ejs to login page also
}
